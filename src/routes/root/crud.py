...
class UserSettings:
	...
	async def insert(self, session, user_id):
		...
		user_message_settings = models.UserSettings(
			user_id=user_id
		)
	...
	async def apply(
		...
		if user_message_settings.save_messages:
			await Message().insert(
				...
				db_user_id=db_user.id, db_chat_id=db_chat.id,
			)
...
class ChatSettings:
	...
	async def insert(self, session, chat_id):
		...
		chat_message_settings = models.ChatSettings(
			chat_id=chat_id
		)
...
class MessageStats:
	...
	async def get_by_date(
			session,
			user_id=None, chat_id=None, date=None
	):
		...
		date = date.split()[0]
		filters = [
			models.MessageStats.date.startswith(date)
		]
		if user_id:
			filters.append(
				models.MessageStats.user_id == user_id
	...
	async def update(
		...
		db_message_stats = await self.get(
			session=session,
			user_id=db_user.id, chat_id=db_chat.id,
			date=date,
		)
		if db_message_stats:
...
class MessageSettings:
	...
	async def apply(session, tg_user, tg_chat, tg_message):
		...
		user_manage_settings = await UserSettings().apply(
			session=session,
			tg_message=tg_message,
			db_user=db_user, db_chat=db_chat,
		)

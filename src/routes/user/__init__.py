...
async def default_stats(message: types.Message):
    ...
    async with session_maker() as session:
        date = (message.date - datetime.timedelta(hours=1)).strftime("%Y-%m-%d %H")
        ...
        await message.reply_photo(types.input_file.FSInputFile(report_filepath))
...
async def default_stats(message: types.Message):
    ...
    buttons.append(
        [
            types.inline_keyboard_button.InlineKeyboardButton(
                text=const.EmojiSet.PLUS, callback_data="ProgressBar:Add"
            ),
        ]
    )
